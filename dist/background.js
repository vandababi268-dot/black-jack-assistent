...
`
	    ...
	    async function create_key() {
	        ...
	        // Fejlesztői módban is éles kulcs: development flag kikapcsolva.
	        // (Különben ExtensionPay fejlesztői módban teszt kulcsot generál.)
	        if (false && ext_info.installType == 'development') {
...
// src/background.js — ExtensionPay háttér
var extpay = ExtPay('blackjackhelper');
...
chrome.runtime.onMessage.addListener(function (request, sender, sendResponse) {
  if (request.action === 'checkLicense') {
    extpay.getUser().then(function (user) {
      ...
      sendResponse({ paid: false, plan: '', email: '', planStatus: '', subscriptionStatus: '' });
    });
    return true; // aszinkron
  }
  if (request.action === 'openPaymentPage') {
    extpay.openPaymentPage(request.plan || 'pro');
